...
// Compact format for the dropdown ("2h", "3d", "now")
...
// Full format for the notifications page ("2h ago", "3d ago", "just now")
export function getRelTimeFull(dateStr: string): string {
  const diffMs = Date.now() - new Date(dateStr).getTime()
  const minutes = Math.floor(diffMs / 60000)
  const hours = Math.floor(minutes / 60)
  const days = Math.floor(hours / 24)
