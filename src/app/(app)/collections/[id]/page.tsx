...
function CollectionItemCard({
  ...
  const hasImage = !!rec.image_url && !imgError
  ...
  return (
    ...
    >
      {/* Remove button — only for own collections */}
      {onRemove && (
        <button
          onClick={e => { e.stopPropagation(); onRemove() }}
          aria-label="Remove from collection"
        ...
        >
          <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="white" strokeWidth="2.8" strokeLinecap="round">
            <path d="M18 6L6 18M6 6l12 12" />
          </svg>
        </button>
      )}
      ...
      {/* Description excerpt */}
      {rec.description && (
        <div style={{ padding: '6px 16px 0', paddingRight: onRemove && !hasImage ? '44px' : '16px' }}>
          <p className="font-body" style={{
      ...
      {/* Image */}
      {hasImage && (
        <div style={{ height: '220px', overflow: 'hidden', position: 'relative', background: theme.colors.surface, marginTop: '8px' }}>
          <RecommendationImage
            fill src={rec.image_url} category={rec.category} alt={rec.title}
            sizes="(max-width: 768px) 100vw, 50vw"
            onFallback={() => setImgError(true)}
            style={{ objectFit: 'contain', background: theme.colors.surface }}
          />
        </div>
      )}

      {/* Title */}
      <div style={{ padding: '8px 16px 14px', paddingRight: onRemove && !hasImage && !rec.description ? '44px' : '16px' }}>
